...
def enhancedFeatureExtractorDigit(datum):
    ...
    marked= util.Counter()
    cc_count = cc(datum, marked)
    features["cc1"] = cc_count > 1
    features["cc2"] = cc_count > 3
    features["cc3"] = cc_count > 5
    return features

def getFeasibleMove(pixel):
    moves = [(0, 1), (1, 0), (0, -1), (-1, 0)]
    feasibleMoves = []
    for move in moves:
        newPixel = (move[0] + pixel[0], move[1] + pixel[1])
        if newPixel[0] >= DIGIT_DATUM_WIDTH or newPixel[0] < 0:
            continue
        if newPixel[1] >= DIGIT_DATUM_HEIGHT or newPixel[1] < 0:
            continue
        feasibleMoves.append(newPixel)
    return feasibleMoves

def dfs(datum, pixel, marked):
    marked[pixel] = 1
    for next_pixel in getFeasibleMove(pixel):
        if marked[next_pixel] or datum.getPixel(next_pixel[0], next_pixel[1]):
            continue
        dfs(datum, next_pixel, marked)
...
def cc(datum, marked):
    ...
    for x in range(DIGIT_DATUM_WIDTH):
        for y in range(DIGIT_DATUM_HEIGHT):
            if marked[(x, y)] or datum.getPixel(x, y):
                continue
            num_cc += 1
            dfs(datum, (x,y), marked)
    return num_cc
